...
echo "Set Release"
curl https://sentry.io/api/0/projects/streamr/streamr-platform/releases/ \
  -X POST \
...
echo "Upload map"
curl https://sentry.io/api/0/projects/streamr/streamr-platform/releases/${TRAVIS_TAG}/files/ \
  -X POST \
  -H "Authorization: Bearer $SENTRY_TOKEN" \
